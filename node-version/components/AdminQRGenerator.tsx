...
export default function AdminQRGenerator({ initialCodes, locale }: AdminQRGeneratorProps) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            console.error(error);
            alert(t.admin.qrGenerator.error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const downloadCSV = () => {
        const headers = `${t.admin.qrGenerator.tableToken},${t.admin.qrGenerator.tableShortCode},${t.admin.qrGenerator.tableUrl},${t.admin.qrGenerator.tableDate}\n`;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="card p-8 border-2 border-primary/10">
                    <h2 className="text-xl font-black mb-6 flex items-center gap-2">
                        ...
                        {t.admin.qrGenerator.newBatch}
                    </h2>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">{t.admin.qrGenerator.quantity}</label>
                            <input
                                type="number"
                                value={quantity}
                        ...
                        >
                            {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : <><QrCode className="w-6 h-6" /> {t.admin.qrGenerator.generateBtn}</>}
                        </button>
                        {success && (
                            <div className="flex items-center gap-2 text-secondary font-bold justify-center animate-in fade-in slide-in-from-top-2">
                                <CheckCircle className="w-5 h-5" /> {t.admin.qrGenerator.success}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 card p-8 bg-gray-900 text-white border-0 shadow-2xl">
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            <h2 className="text-2xl font-black mb-2 italic">{t.admin.qrGenerator.dashboardTitle}</h2>
                            <p className="text-gray-400 font-medium">{t.admin.qrGenerator.dashboardDesc}</p>
                        </div>
                        <button
                            onClick={downloadCSV}
                        ...
                        >
                            <Download className="w-5 h-5" /> {t.admin.qrGenerator.exportBtn}
                        </button>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
                            <p className="text-gray-400 text-xs font-black uppercase tracking-widest mb-1">{t.admin.qrGenerator.totalUnassigned}</p>
                            <p className="text-4xl font-black text-primary">{codes.length}</p>
                        </div>
                        <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
                            <p className="text-gray-400 text-xs font-black uppercase tracking-widest mb-1">{t.admin.qrGenerator.outputFormat}</p>
            ...
            <div className="card overflow-hidden">
                <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <h3 className="font-black flex items-center gap-2">
                        ...
                        {t.admin.qrGenerator.lastGenerated}
                    </h3>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-white border-b border-gray-100">
                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-gray-400">{t.admin.qrGenerator.tableToken}</th>
                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-gray-400">{t.admin.qrGenerator.tableShortCode}</th>
                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-gray-400">{t.admin.qrGenerator.tableUrl}</th>
                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-gray-400">{t.admin.qrGenerator.tableDate}</th>
                                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-gray-400">{t.admin.qrGenerator.tableAction}</th>
                            </tr>
                        </thead>
