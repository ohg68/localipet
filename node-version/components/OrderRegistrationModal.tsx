...
export default function OrderRegistrationModal({ animalId, animalName, currentFood, orgId, locale }: OrderRegistrationModalProps) {
    ...
    if (!isOpen) {
        return (
            ...
            >
                {t.vet.orderModal.newSale} <ShoppingBag className="w-3 h-3" />
            </button>
        );
    }
            ...
            <div className="bg-white w-full max-w-xl rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col relative border border-white/20 animate-in zoom-in-95 duration-500">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-xl font-black italic tracking-tight uppercase">
                                {t.vet.orderModal.title.split(" ").map((word, i) =>
                                    word.toLowerCase() === "alimento" || word.toLowerCase() === "alimento" ? (
                                        <span key={i} className="text-primary ml-1">{word} </span>
                                    ) : (
                ...
                {sent ? (
                    <div className="p-20 text-center flex flex-col items-center animate-in zoom-in-90 duration-500">
                        ...
                        <h4 className="text-3xl font-black italic mb-4">{t.vet.orderModal.success}</h4>
                        <p className="text-gray-400 font-bold max-w-xs mx-auto italic">
                            {t.vet.orderModal.successDesc.replace("{name}", animalName)}
                        </p>
                    </div>
                ) : (
                    <div className="p-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        <div className="mb-8 p-6 bg-primary/5 rounded-3xl border border-primary/10">
                            <p className="text-xs font-black uppercase tracking-widest text-primary mb-2">{t.vet.orderModal.patientSummary}</p>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                                <label className="text-xs font-black uppercase tracking-widest text-gray-400 block ml-1">{t.vet.orderModal.foodBrand}</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder={t.vet.orderModal.foodPlaceholder}
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <label className="text-xs font-black uppercase tracking-widest text-gray-400 block ml-1">{t.vet.orderModal.packageSize}</label>
                                    <div className="relative">
                                ...
                                <div className="space-y-4">
                                    <label className="text-xs font-black uppercase tracking-widest text-gray-400 block ml-1">{t.vet.orderModal.dailyGrams}</label>
                                    <div className="relative">
                            ...
                            <div className="bg-orange-50 p-6 rounded-3xl border border-orange-100 flex gap-4">
                                ...
                                <p className="text-xs font-bold text-orange-700 leading-relaxed italic">
                                    {t.vet.orderModal.alertNotice
                                        .replace("{name}", animalName)
                                        .replace("{days}", (packageSize * 1000 / dailyGrams).toFixed(0))
                                        .replace("{alertDays}", ((packageSize * 1000 / dailyGrams) - 7).toFixed(0))}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="pt-8">
                            ...
                            >
                                {loading ? t.vet.orderModal.processing : (
                                    <>{t.vet.orderModal.submit} <CheckCircle2 className="w-5 h-5" /></>
                                )}
                            </button>
                        </div>
                    </div>
